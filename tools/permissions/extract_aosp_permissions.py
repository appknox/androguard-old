...
AOSP_PERMISSION_MODULE_PATH = "../../androguard/core/api_specific_resources/aosp_permissions/"

# hope Android will get 3digit version number :)
SDK_VERSION_PATTERN = re.compile(
    "\s*PLATFORM_SDK_VERSION\s*:=\s*(?P<sdk_version>\d{1,3})\s*")
# just to add as a comment from which version the parsing has happened
PLATFORM_VERSION_PATTERN = re.compile(
    "\s*PLATFORM_VERSION\s*:=\s*(?P<platform_version>.+)\s*")
...
def getPlatformVersions(aosp_root_dir):
    ...
    version_file_path = os.path.join(
        aosp_root_dir, PLATFORM_VERSIONS_FILE_REL_PATH)
...
def get_permission_details(manifest_dir):
    ...
    for i in manifest_document.getElementsByTagName("permission"):
        ...
        perms[name] = {"label": label, "description": description,
                       "protectionLevel": protection_level, "permissionGroup": permission_group}
...
print("Detected sdk_version [%s], platform_version [%s]..." % (
...
perms_module_path = os.path.join(
    AOSP_PERMISSION_MODULE_PATH, perms_module_name)
if os.path.exists(perms_module_path):
    print("API specific file for this version already exists!")
    print("If you want create a file for newer version, please, delete file: %s" %
          perms_module_path)
    exit(1)
...
dirs_with_manifest = get_all_dirs_with_manifest(
    root_dir_path=PATH_TO_AOSP_ROOT)
...
with codecs.open(perms_module_path, 'w', 'utf-8') as perm_py_module:
    ...
    perm_py_module.write(
        "### Extracted from platform version: %s \n" % platform_version)
    perm_py_module.write("#################################################\n")
